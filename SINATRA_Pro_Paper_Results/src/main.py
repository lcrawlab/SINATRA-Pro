...
parser.add_argument('-sg','--single', dest='single', action='store_true')
...
parser.set_defaults(parallel=False,single=False,verbose=False)
...
single = args.single ## single="True" is for single run purpose, "False" for duplicate runs purpose which groups and names file with the frame offset.
...
if single:
    ...
    directory_msh = "%s/msh/"%(directory)
else:
    directory_pdb_A = "%s/pdb/%s_offset_%d/"%(directory,protA,offset)
    directory_pdb_B = "%s/pdb/%s_offset_%d/"%(directory,protB,offset)
    reference_pdb_file = "%s/%s_frame0.pdb"%(directory_pdb_A,protA) ## which pdb to use for visualization
    directory_msh = "%s/msh_offset_%d/"%(directory,offset)
...
X, y, not_vacuum = compute_ec_curve_folder(protA,protB,directions,
        ...
        sm_radius=sm_radius,
        directory_mesh_A = "%s/%s_%.1f"%(directory_msh,protA,sm_radius), 
        directory_mesh_B = "%s/%s_%.1f"%(directory_msh,protB,sm_radius), 
        parallel=parallel, 
        n_core=n_core, 
        verbose=verbose)
...
vert_prob = reconstruct_on_multiple_mesh(protA,protB,directions,
        ...
        n_filtration=n_filtration,
        sm_radius=sm_radius,
        directory_mesh="%s/%s_%.1f"%(directory_msh,protA,sm_radius),
        verbose=verbose)
