...
for mutant in mutants:
    ...
    for i in range(1,101):
        raw = np.loadtxt('ec_sphere/%s/%s_chimera_%d.dat'%(mutant,mutant,i))
        if i == 1:
            x = raw[0]
        y = raw[1:]
        data.append(y)
...
for idir in range(n_direction):
    ...
    plt.tight_layout()
    plt.savefig('plot/compare/normed_ec_curve_%d.pdf'%idir)
    plt.show()
    plt.close()
